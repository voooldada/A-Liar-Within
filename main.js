...
const corruptionMessage = document.getElementById('corruption-message');
...
const translations = {
  pt: {
    ...
    heroSocials: 'Veja as nossas redes sociais! @vooolcanicalgames no 📸 Instagram, 🎵 TikTok e 🐦 Twitter!',
    ...
    continueToGame: 'Continuar para o jogo',
    ...
    dayNarrationHeader: 'Acontecimento da Noite',
    alivePlayersTitle: 'Jogadores Vivos',
    dayNarrationEmpty: 'Sem eventos noturnos.',
    ...
    assassinWakeupMessage: 'O assassino acorda e escolhe alguém para matar',
    assassinWakeupShort: 'O assassino acorda e age',
    traitorWakeupMessage: 'O traidor acorda e escolhe alguém para matar',
    doctorWakeupMessage: 'O médico escolhe que alma irá salvar',
    detectiveWakeupMessage: 'O detetive acorda e investiga o caso',
    traitorNeighborhoodWarning: 'Existe um traidor na vizinhança',
    ...
    corruptionMessage: 'O sistema está corrompido... Prepare-se para a revelação de papéis.',
    // 🎭 Frases da narração do dia
    narratorDayDeathSingle: 'Ouve uma morte na noite passada...',
    narratorDayDeathReveal: 'O jogador {name} foi assassinado.',
    narratorDayNoDeaths: 'Estranhamente...',
    narratorDayNoDeathsReveal: 'Não ocorreram mortes na noite passada.',
    narratorDayDeathSaved: 'Na noite passada, o jogador {name} quase foi assassinado...',
    narratorDayDeathSavedReveal: 'Mas graças ao médico ele foi salvo.',
    narratorDayTwoDeaths: 'Duas mortes aconteceram na cidade na noite passada...',
    narratorDayTwoDeathsReveal: 'Jogador {names} infelizmente foram assassinados.',
    narratorDayMedicNoSave: 'O médico não salvou ninguém na noite passada.',
    narratorDayMedicSave: 'O médico salvou o jogador {name}.'
  },
  en: {
    gameTitle: 'A Liar Within',
    play: 'Play',
    subtitle: 'Can you find the escape route?',
    heroSocials: 'Check out our social media! @vooolcanicalgames on 📸 Instagram, 🎵 TikTok and 🐦 Twitter!',
    ...
    dayNarrationHeader: 'Night Events',
    alivePlayersTitle: 'Alive Players',
    dayNarrationEmpty: 'No night events.',
    ...
    assassinWakeupMessage: 'The assassin wakes up and chooses someone to kill',
    assassinWakeupShort: 'The assassin wakes up and acts',
    traitorWakeupMessage: 'The traitor wakes up and chooses someone to kill',
    doctorWakeupMessage: 'The doctor chooses which soul to save',
    detectiveWakeupMessage: 'The detective wakes up and investigates the case',
    traitorNeighborhoodWarning: 'There is a traitor in the neighborhood',
    ...
    corruptionMessage: 'System corruption detected... Prepare for the role reveal.',
    // 🎭 Day narration phrases
    narratorDayDeathSingle: 'A death was heard in the night...',
    narratorDayDeathReveal: 'Player {name} was assassinated.',
    narratorDayNoDeaths: 'Strangely...',
    narratorDayNoDeathsReveal: 'No deaths occurred during the night.',
    narratorDayDeathSaved: 'Last night, player {name} was almost assassinated...',
    narratorDayDeathSavedReveal: 'But thanks to the doctor, they were saved.',
    narratorDayTwoDeaths: 'Two deaths occurred in the city last night...',
    narratorDayTwoDeathsReveal: 'Players {names} were unfortunately assassinated.',
    narratorDayMedicNoSave: 'The doctor did not save anyone last night.',
    narratorDayMedicSave: 'The doctor saved player {name}.'
  }
};
...
// Mostra tela de corrupção antes da revelação de roles
function showCorruptionScreen() {
  hideAllScreens();
  if (corruptionMessage) {
    corruptionMessage.textContent = translateString('corruptionMessage');
  }
...
function showAssassinScreen() {
  ...
  if (effectiveRole !== ROLES.ASSASSIN || !isCurrentPlayerAlive()) {
    ...
    showNightAnnouncement(translateString('assassinWakeupShort'), false);
    return;
  }
...
function showDayDiscussion(deathAnnouncement = '', options = {}) {
  clearNightAnnouncementTimer();
  ...
  renderDayNarration(announcement);
...
function shouldContinueToGameAfterDeath(role) {
  const alivePlayers = Object.entries(gameRoles.roles || {}).filter(([, roleData]) => roleData.alive !== false);
  ...
  const assassinKilledByVote = role === ROLES.ASSASSIN;

  return !(assassinKilledByVote && !traitorAlive) && !(assassinAlive && aliveCount <= 2);
}
...
function showDeathNotification(deadPlayer, role, options = {}) {
  ...
  if (deathContinueButton) {
    const continueToGame = shouldContinueToGameAfterDeath(role);
    deathContinueButton.textContent = translateString(continueToGame ? 'continueToGame' : 'continueToLobby');
    deathContinueButton.dataset.mode = continueToGame ? 'game' : 'lobby';
  }
...
/**
 * 🎭 Gera a narração do dia com delays baseado nos eventos da noite
 * @param {Array} killedTargets - Array com IDs dos jogadores que morreram
 * @param {String} assassinTarget - ID do alvo do assassino
 * @param {String} traitorTarget - ID do alvo do traidor  
 * @param {String} doctorSave - ID do jogador salvo pelo médico
 * @returns {Promise<String>} HTML com a narração completa com delays
 */
async function generateDayNarration(killedTargets = [], assassinTarget = null, traitorTarget = null, doctorSave = null) {
  const deathCount = killedTargets.length;
  const allTargets = [assassinTarget, traitorTarget].filter(Boolean);
  const targetsCount = allTargets.length;
  const wasSomeoneSaved = doctorSave ? true : false;
  const didDoctorSaveKilledPerson = killedTargets.includes(doctorSave);
  
  let htmlNarration = '';
  
  // Construir narração baseado nos cenários
  if (deathCount === 0 && targetsCount === 0) {
    // Cenário 2: Ninguém foi alvo, ninguém morreu
    htmlNarration = `
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 0.3s forwards;">
        <p class="narration-text narration-suspense">${translateString('narratorDayNoDeaths')}</p>
      </div>
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 2.8s forwards;">
        <p class="narration-text narration-neutral">${translateString('narratorDayNoDeathsReveal')}</p>
      </div>
    `;
    
    if (wasSomeoneSaved) {
      const savedPlayer = currentRoomData.players?.[doctorSave];
      const savedPlayerName = savedPlayer?.name || 'Desconhecido';
      htmlNarration += `
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 4.3s forwards;">
          <p class="narration-text narration-save">${translateString('narratorDayMedicSave', { name: savedPlayerName })}</p>
        </div>
      `;
    } else {
      htmlNarration += `
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 4.3s forwards;">
          <p class="narration-text narration-suspense">${translateString('narratorDayMedicNoSave')}</p>
        </div>
      `;
    }
  } else if (deathCount === 0 && targetsCount > 0) {
    // Cenário 3: Alguém foi atacado mas foi salvo (todos os ataques foram bloqueados)
    const targetsSaved = allTargets.filter(t => t === doctorSave);
    
    if (targetsSaved.length > 0) {
      // Todos foram salvos - mostrar cenário de salvação
      const savedPlayer = currentRoomData.players?.[targetsSaved[0]];
      const savedPlayerName = savedPlayer?.name || 'Desconhecido';
      
      htmlNarration = `
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 0.3s forwards;">
          <p class="narration-text narration-suspense">${translateString('narratorDayDeathSaved', { name: savedPlayerName })}</p>
        </div>
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 2.8s forwards;">
          <p class="narration-text narration-save">${translateString('narratorDayDeathSavedReveal')}</p>
        </div>
      `;
    } else {
      // Alguém foi atacado mas médico salvou outro
      htmlNarration = `
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 0.3s forwards;">
          <p class="narration-text narration-neutral">${translateString('narratorDayNoDeaths')}</p>
        </div>
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 2.8s forwards;">
          <p class="narration-text narration-neutral">${translateString('narratorDayNoDeathsReveal')}</p>
        </div>
  ...
  } else if (deathCount === 1 && !didDoctorSaveKilledPerson) {
    // Cenário 1: Uma morte (assassino matou, ninguém salvou)
    const killedPlayer = currentRoomData.players?.[killedTargets[0]];
    const killedPlayerName = killedPlayer?.name || 'Desconhecido';
    
    htmlNarration = `
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 0.3s forwards;">
        <p class="narration-text narration-suspense">${translateString('narratorDayDeathSingle')}</p>
      </div>
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 2.8s forwards;">
        <p class="narration-text narration-death">${translateString('narratorDayDeathReveal', { name: killedPlayerName })}</p>
      </div>
    `;
    
    // Se médico salvou alguém diferente
    if (wasSomeoneSaved && !killedTargets.includes(doctorSave)) {
      const savedPlayer = currentRoomData.players?.[doctorSave];
      const savedPlayerName = savedPlayer?.name || 'Desconhecido';
      htmlNarration += `
        <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 4.3s forwards;">
    ...
    // Cenário 5: Duas mortes (assassino + traidor mataram)
    const names = killedTargets.map(id => currentRoomData.players?.[id]?.name || 'Desconhecido').join(' e ');
    
    htmlNarration = `
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 0.3s forwards;">
        <p class="narration-text narration-suspense">${translateString('narratorDayTwoDeaths')}</p>
      </div>
      <div class="narration-line" style="opacity: 0; animation: fadeInUp 0.5s ease-out 2.8s forwards;">
        <p class="narration-text narration-death">${translateString('narratorDayTwoDeathsReveal', { names: names })}</p>
      </div>
    `;
  }
  
  return htmlNarration;
}

// Renderiza lista de jogadores vivos
function renderAlivePlayersList() {
  ...
  container.innerHTML = `
    <h3>${translateString('alivePlayersTitle')}</h3>
    <ul>
...
function renderDayNarration(narrationHTML = '', options = {}) {
  const container = document.getElementById('day-narration-panel');
  if (!container) return;
  const content = narrationHTML || (!options.hideEmptyMessage ? `<div class="day-death-announcement-empty">${translateString('dayNarrationEmpty')}</div>` : '');
  container.innerHTML = `
    <div class="day-narration-header">${translateString('dayNarrationHeader')}</div>
    ${content}
  `;
}
...
async function initializeGameSystems() {
  ...
  gameRoles.listenGameState((data) => {
    ...
    const currentPhase = currentRoomData?.gameState?.phase || data.gameState?.phase;
    if (previousRole !== currentPlayerRole) {
      console.log('🔁 Role updated, current phase:', currentPhase, 'new role:', currentPlayerRole);
      if (currentPhase && currentPhase !== window.lastProcessedPhase) {
        console.log('🔁 Phase differs from last processed phase, handling update after role sync:', currentPhase);
        void handlePhaseUpdate(currentPhase, currentRoomData?.gameState || data.gameState);
      } else {
        console.log('⏭️ Skipping phase re-handle after role sync because phase is already processed or unavailable');
      }
    }
  });
...
if (votingSkipButton) {
  votingSkipButton.addEventListener('click', async () => {
    if (!currentPlayerId || !gameVoting) {
      console.warn('⚠️ Cannot continue voting - missing player or voting system');
      return;
    }

    const playerRole = gameRoles.roles[currentPlayerId];
    if (playerRole?.alive === false) {
      console.log('💀 Player is dead, cannot continue voting');
      return;
    }

    if (gameVoting.votingContinueActions?.[currentPlayerId] === true) {
      console.log('🔁 Player already requested continue in voting');
      return;
    }

    await gameVoting.continueVoting(currentPlayerId);
    updateVotingContinueButton();
  });
}
...
if (deathContinueButton) {
  deathContinueButton.addEventListener('click', async () => {
    ...
    const continueToGame = deathContinueButton.dataset.mode !== 'lobby';
  ...
  switch (phase) {
    ...
    case PHASES.NIGHT:
      ...
      if (pendingTraitorNeighborhoodAnnouncement && currentPlayerRole === ROLES.TRAITOR) {
        ...
        showNightAnnouncement(translateString('traitorNeighborhoodWarning'), () => {
          ...
          if (effectiveRole === ROLES.ASSASSIN && isCurrentPlayerAlive()) {
            ...
            try {
              ...
              if (audioPermissionGranted && assassinWakeupAudio) {
                playOverlayAudio(assassinWakeupAudio, nightMusic, 0.15);
                assassinWakeupAudio.onended = () => {
                  showNightScreen(translateString('assassinWakeupMessage'), false);
                };
              } else {
                showNightScreen(translateString('assassinWakeupMessage'), false);
              }
        ...
        if (audioPermissionGranted && traitorWakeupAudio) {
          ...
          if (effectiveRole === ROLES.TRAITOR && isCurrentPlayerAlive()) {
            ...
            traitorWakeupAudio.onended = () => {
              showNightScreen(translateString('traitorWakeupMessage'), false);
            };
          }
        } else {
          if (effectiveRole === ROLES.TRAITOR && isCurrentPlayerAlive()) {
      ...
      if (effectiveRole === ROLES.DOCTOR && isCurrentPlayerAlive()) {
        ...
        try {
          ...
          if (audioPermissionGranted && doctorWakeupAudio) {
            ...
            // Wait for audio to end before showing night screen
            doctorWakeupAudio.onended = () => {
              showNightScreen(translateString('doctorWakeupMessage'), false);
            };
          } else {
            showNightScreen(translateString('doctorWakeupMessage'), false);
          }
      ...
      if (effectiveRole === ROLES.DETECTIVE && isCurrentPlayerAlive()) {
        ...
        try {
          ...
          if (audioPermissionGranted && detectiveWakeupAudio) {
            ...
            // Wait for audio to end before showing night screen
            detectiveWakeupAudio.onended = () => {
              showNightScreen(translateString('detectiveWakeupMessage'), false);
            };
          } else {
            showNightScreen(translateString('detectiveWakeupMessage'), false);
          }
    ...
    case PHASES.DAY:
      {
        ...
        const dayFollowsNight = [
          PHASES.ROLE_REVEAL,
          PHASES.NIGHT,
          PHASES.ASSASSIN_TURN,
          PHASES.TRAITOR_TURN,
          PHASES.DOCTOR_TURN,
          PHASES.DETECTIVE_TURN
        ].includes(previousPhase);

        if (!dayFollowsNight) {
          console.log('⏭️ Skipping night action processing because DAY follows non-night phase:', previousPhase);
          showDayDiscussion('', { hideEmptyMessage: true });
          return;
        }
...
function setupGameListeners() {
  ...
  votingContinueActionsUnsubscribe = gameVoting.listenVotingContinueActions(async (actions) => {
    ...
    if (gameVoting.hasAllVotingContinued(aliveIds)) {
      await gameVoting.clearVotingContinueActions();
      console.log('✅ All players continued from voting, returning to day discussion');
      await gameTurns.setPhase(PHASES.DAY);
    }
  });
...
function initializeChatListeners() {
  ...
  if (openSecretChatButton) {
    openSecretChatButton.addEventListener('click', () => {
      ...
      // 🎭 Abrir painel de máscara se for traidor
      if (currentPlayerRole === ROLES.TRAITOR) {
        if (maskPanel) maskPanel.classList.remove('hidden');
        if (maskPanelFullscreen) maskPanelFullscreen.classList.remove('hidden');
      }
    });
  }
...
async function processNightActions() {
  ...
  let gameStateData = currentRoomData?.gameState || {};
  try {
    if (currentRoom) {
      const roomStateSnapshot = await get(ref(getDatabase(), `rooms/${currentRoom}/gameState`));
      if (roomStateSnapshot.exists()) {
        gameStateData = roomStateSnapshot.val();
        if (currentRoomData) currentRoomData.gameState = gameStateData;
        if (gameRoles) gameRoles.gameState = gameStateData;
      }
    }
  } catch (error) {
    console.warn('⚠️ Falha ao atualizar gameState antes de processar noite:', error);
  }
  ...
  try {
    ...
    if (killedTargets.length > 0) {
      ...
      // 🎭 Gerar narração com delays
      const narrationHTML = await generateDayNarration(killedTargets, assassinTarget, traitorTarget, doctorSave);
      ...
      if (killedTargets.includes(currentPlayerId)) {
        showDayDiscussion(narrationHTML);
        setTimeout(() => {
          returnToStartScreen();
        }, 3500);
        return;
      ...
      // 🎭 Gerar narração para quando ninguém morre
      const narrationHTML = await generateDayNarration([], assassinTarget, traitorTarget, doctorSave);
